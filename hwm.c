...
void cycle_tile(Window win, int dir)
{
	Tile *swap;
	int src = get_tile(win) - tile;
	int dest = src + 1;

	if (src < 0 || !tile[src]->is_tiled)
		return;
		...
		case KeyPress:
			if (evt.xkey.subwindow != None) {
				if (evt.xkey.keycode == XKeysymToKeycode(
					...
					tile_on(evt.xkey.subwindow);
					tile_all();
				} else if (evt.xkey.keycode == XKeysymToKeycode(
					dpy, XStringToKeysym("Prior"))
				) {
					cycle_tile(evt.xkey.subwindow, -1);
					tile_all();
					XSetInputFocus(dpy, evt.xkey.subwindow, RevertToParent, CurrentTime);
				} else if (evt.xkey.keycode == XKeysymToKeycode(
					dpy, XStringToKeysym("Next"))
				) {
					cycle_tile(evt.xkey.subwindow, 1);
					tile_all();
					XSetInputFocus(dpy, evt.xkey.subwindow, RevertToParent, CurrentTime);
